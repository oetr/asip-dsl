(module racket-vhdl racket
  ...
  (define-syntax define-opcodes
    (syntax-rules ()
      [(_ instructions OPERATION-WIDTH)
  ...
  ;; Machine Code
  ;; ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  (define (convert-machine-code mc INSTRUCTION-WIDTH)
  ...
  ;; Machine Code
  ;; ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  (define (generate-library-file mc instructions asip-constants)
    (define OPERATION-WIDTH (list-ref asip-constants 0))
    (define REGISTER-WIDTH (list-ref asip-constants 1))
    (define REGISTER-N-WIDTH (list-ref asip-constants 2))
    (define LINE-N-WIDTH (list-ref asip-constants 3))
    (define INSTRUCTION-WIDTH (list-ref asip-constants 4))
    (define nl "\n")
    (~a
     ;; make a header
     "library ieee;" nl
     ...
     (convert-constants* OPERATION-WIDTH
                         ...
                         "  -- Opcodes"nl
                         "  --------------------------------------------------------------------" nl
                         (define-opcodes instructions OPERATION-WIDTH)
                         "  --------------------------------------------------------------------" nl
                         "  -- Registers"nl
                         "  --------------------------------------------------------------------" nl
                         ...
                         "  -- Code"nl
                         "  --------------------------------------------------------------------" nl
                         (convert-machine-code mc INSTRUCTION-WIDTH)
                         nl
                         ;; finish library
                         "end package test;"nl))
  ...
  ;; ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  (define (generate-main-file i/o user-registers instructions asip-constants )
    (define OPERATION-WIDTH (list-ref asip-constants 0))
    (define REGISTER-WIDTH (list-ref asip-constants 1))
    (define REGISTER-N-WIDTH (list-ref asip-constants 2))
    (define LINE-N-WIDTH (list-ref asip-constants 3))
